---
title: "Week 03 Functions, Best-coding Practices, and Debugging Intro"
author: "Clay Wright"
format: html
toc: true
editor: 
...
-   Organize code into logical blocks and use comments to help
    understand code
-   Use RStudios formatting tools to avoid common errors
...
The `return(CV.value)` indicates what is returned by the function. If there is no `return` function the result of the last line is returned.
...
-   To install CRAN packages you can 
    -   use the `install.packages` function with the argument being a character string of the package name
    -   use the Packages pane (which runs `install.packages` for you)
-   Bioconductor and Github packages are a bit more tricky requiring packages to install their packages
...
-   Always make sure there is a `>` in the console! A `+` means the last line wasn't completed. 
